...
public class    DynamicLineChartManager {
    ...
    */
     ...
     *               position 添加的列表
     */
    public void addMyEntry(float number,int position)  {
        ...
        if (lineDataSets != null) {
            ...
            Entry entry = new Entry(lineDataSet.getEntryCount(), number);
            lineData.addEntry(entry, position);
            lineData.notifyDataChanged();
            lineChart.notifyDataSetChanged();
            lineChart.setVisibleXRangeMaximum(6);
            lineChart.moveViewToX(lineData.getEntryCount() - 5);
     ...
     * @param
     */
    public void deleteMyEntry()  {


        if (lineDataSets != null) {
            for (int i = 0; i < lineDataSets.size(); i++) {
                if (lineData != null && lineDataSets.get(i).getEntryCount() >= 1){
                    Entry lastEntry = lineDataSets.get(i).getEntryForIndex(lineDataSets.get(i).getEntryCount()-1);
                    lineData.removeEntry(lastEntry, i);
                    lineData.notifyDataChanged();
                    lineChart.notifyDataSetChanged();
                    lineChart.setVisibleXRangeMaximum(6);
                    lineChart.moveViewToX(lineData.getEntryCount() - 5);
    ...
    public void clearData(){
        if (lineDataSets != null) {
            int len = 0;
            for (int i = 0; i < lineDataSets.size(); i++) {         //找到数据最多的曲线
                if (lineDataSets.get(i).getEntryCount() > len) {
                    len = lineDataSets.get(i).getEntryCount();
                }
            }
            for (int j = 0; j < len; j++) {
                deleteMyEntry();
